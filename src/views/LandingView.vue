...
<template>
  <div class="min-h-screen bg-white dark:bg-[#121214] text-slate-900 dark:text-white selection:bg-blue-600 selection:text-white font-sans overflow-x-hidden transition-colors duration-300">
      ...
      <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
          <button 
            @click="toggleTheme" 
            title="Toggle theme"
            class="p-2.5 rounded-xl bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-600 dark:text-white transition-all cursor-pointer border border-transparent dark:border-white/5"
          >
          ...
          <button 
            @click="goToLogin" 
            class="text-sm font-bold bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-900 dark:text-white px-5 py-2.5 rounded-xl transition-all border border-transparent dark:border-white/5 cursor-pointer shadow-sm hover:shadow"
          >
            Member Login
          </button>
        </div>
    ...
    <main class="relative pt-32 pb-16 lg:pt-44 lg:pb-24 overflow-hidden flex items-center">
      ...
      <div class="absolute inset-0 z-0">
        <img 
          ...
          class="w-full h-full object-cover object-center opacity-35 dark:opacity-40 scale-105" 
        />
        <div class="absolute inset-0 bg-gradient-to-t from-white via-white/85 dark:from-[#121214] dark:via-[#121214]/85 to-transparent transition-colors duration-300"></div>
        <div class="absolute inset-0 bg-gradient-to-r from-white via-white/75 dark:from-[#121214] dark:via-[#121214]/75 to-transparent transition-colors duration-300"></div>
      </div>

      <div class="relative z-10 max-w-7xl mx-auto px-6 w-full">
        <div class="max-w-3xl">
          <div class="inline-flex items-center space-x-2 bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest mb-6">
          ...
          <h1 class="text-5xl sm:text-6xl lg:text-7xl font-black text-slate-900 dark:text-white leading-[1.08] tracking-tight mb-6 drop-shadow-sm">
            Symphonic <br/>
            <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-600 dark:from-blue-400 dark:via-indigo-400 dark:to-sky-400">
              Precision.
            </span>
          </h1>
          
          <p class="text-lg sm:text-xl text-slate-700 dark:text-neutral-300 font-medium leading-relaxed max-w-2xl">
    ...
    <section class="py-16 lg:py-24 relative z-10 bg-slate-50/70 dark:bg-[#151518]/90 border-t border-slate-200/80 dark:border-neutral-800/80 transition-colors duration-300">
      <div class="max-w-7xl mx-auto px-6">
        
        <!-- Section Header -->
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-12">
          <div>
            <div class="inline-flex items-center space-x-2 text-blue-600 dark:text-blue-400 text-xs font-black uppercase tracking-widest mb-2 font-mono">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div class="lg:col-span-6 xl:col-span-7 flex flex-col items-center">
            ...
            <div class="relative w-full flex items-center justify-center min-h-[440px] sm:min-h-[480px] overflow-hidden py-4 select-none">
              ...
              <button 
                ...
                class="absolute left-2 sm:left-4 z-30 w-11 h-11 sm:w-13 sm:h-13 rounded-2xl bg-white/90 dark:bg-[#1c1c21]/90 backdrop-blur-md border border-slate-200 dark:border-white/10 text-slate-700 dark:text-white flex items-center justify-center shadow-xl hover:scale-110 active:scale-95 hover:bg-blue-600 hover:text-white hover:border-blue-600 dark:hover:bg-blue-600 transition-all cursor-pointer group"
              >
              ...
              <div class="flex items-center justify-center space-x-3 sm:space-x-5 w-full">
                ...
                <!-- PREVIOUS CARD (Blurred Preview on Left) -->
                <div 
                  @mouseenter="onHoverAdjacent(prevIndex)"
                  @mouseleave="cancelHover"
                  @click="prevOfficer"
                  class="hidden sm:flex flex-col relative w-36 md:w-44 h-[360px] md:h-[390px] rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 transform scale-95 border border-slate-300 dark:border-white/10 shadow-lg group opacity-60 hover:opacity-95 filter blur-[1.5px] hover:blur-none"
                >
                  <img 
                    :src="officers[prevIndex].image" 
                    :alt="officers[prevIndex].name"
                    class="w-full h-full object-cover object-top brightness-90 group-hover:scale-105 transition-all duration-500"
                  />
                  <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent"></div>
                  
                  <!-- Top Preview Tag -->
                  <div class="absolute top-3 left-3 z-10">
                    <span class="px-2.5 py-1 text-[10px] font-black uppercase tracking-wider bg-black/60 backdrop-blur-md text-white/90 rounded-md border border-white/20">
                ...
                >
                  ...
                  <!-- Card Center Holographic Accent -->
                ...
                <!-- NEXT CARD (Blurred Preview on Right) -->
                <div 
                  @mouseenter="onHoverAdjacent(nextIndex)"
                  @mouseleave="cancelHover"
                  @click="nextOfficer"
                  class="hidden sm:flex flex-col relative w-36 md:w-44 h-[360px] md:h-[390px] rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 transform scale-95 border border-slate-300 dark:border-white/10 shadow-lg group opacity-60 hover:opacity-95 filter blur-[1.5px] hover:blur-none"
                >
              ...
              <button 
                ...
                class="absolute right-2 sm:right-4 z-30 w-11 h-11 sm:w-13 sm:h-13 rounded-2xl bg-white/90 dark:bg-[#1c1c21]/90 backdrop-blur-md border border-slate-200 dark:border-white/10 text-slate-700 dark:text-white flex items-center justify-center shadow-xl hover:scale-110 active:scale-95 hover:bg-blue-600 hover:text-white hover:border-blue-600 dark:hover:bg-blue-600 transition-all cursor-pointer group"
              >
                <ChevronRight class="w-6 h-6 group-hover:translate-x-0.5 transition-transform" />
              </button>
            </div>

            <!-- Game Character Slot Bar (Roster Thumbnails) -->
            <div class="w-full max-w-lg mt-6 bg-white/70 dark:bg-[#18181b]/70 backdrop-blur-xl p-2.5 rounded-2xl border border-slate-200 dark:border-neutral-800 shadow-lg">
              <div class="flex items-center justify-between gap-1.5 sm:gap-2">
                <button
                  ...
                  :class="[
                    'flex-1 flex flex-col items-center py-2 px-1 sm:px-2 rounded-xl transition-all cursor-pointer relative',
                    selectedIndex === idx 
                      ? 'bg-blue-600 text-white shadow-md shadow-blue-600/30 scale-[1.03]' 
                      : 'hover:bg-slate-100 dark:hover:bg-white/5 text-slate-600 dark:text-neutral-400 opacity-70 hover:opacity-100'
                  ]"
                >
                  <div class="relative w-8 h-8 sm:w-9 sm:h-9 rounded-lg overflow-hidden mb-1 border"
                    :class="selectedIndex === idx ? 'border-white ring-2 ring-white/30' : 'border-transparent'"
                  >
                    <img :src="officer.image" :alt="officer.name" class="w-full h-full object-cover object-top" />
                  </div>
                  <span class="text-[9px] sm:text-[10px] font-black tracking-wider uppercase truncate max-w-full">
                    {{ officer.titleCode.replace('BAND ', '') }}
                  </span>
                  <span 
                    v-if="selectedIndex === idx"
                    class="absolute -top-1 right-1 w-2 h-2 rounded-full bg-emerald-400 ring-2 ring-blue-600"
                  ></span>
                </button>
              </div>
            </div>
            ...
            <!-- Interactive Hint -->
            <div class="mt-3 flex items-center space-x-2 text-slate-400 dark:text-neutral-500 text-[11px] font-medium font-mono">
              <span>← Hover cards or click arrows to switch officers →</span>
            </div>
          ...
          <div class="lg:col-span-6 xl:col-span-5">
            <div class="p-8 sm:p-10 rounded-3xl bg-white dark:bg-[#18181b] border border-slate-200 dark:border-neutral-800 shadow-2xl relative overflow-hidden transition-all duration-300">
              ...
              <!-- Top Status HUD -->
              <div class="flex items-center justify-between pb-6 border-b border-slate-100 dark:border-neutral-800/80 mb-6">
                ...
                <span class="text-xs font-mono font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 px-2.5 py-1 rounded-md border border-blue-200 dark:border-blue-500/20">
              ...
              <div class="mb-4">
                <div class="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black text-xs px-3 py-1.5 rounded-lg uppercase tracking-wider mb-3 shadow-md shadow-blue-500/20">
                  ...
                  <span class="opacity-60 font-normal">({{ currentOfficer.titleCode }})</span>
                </div>
                
                <h3 class="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight mb-2">
                  {{ currentOfficer.name }}
                </h3>

                <p class="text-sm font-bold text-blue-600 dark:text-blue-400 flex items-center">
              ...
              <!-- Quote Card -->
              <div class="p-4 rounded-2xl bg-slate-50 dark:bg-neutral-900/70 border border-slate-200/80 dark:border-neutral-800 mb-6 italic text-sm text-slate-700 dark:text-neutral-300 leading-relaxed font-medium">
              ...
              <!-- Game Character Stat HUD (2x2 Grid) -->
              <div class="grid grid-cols-2 gap-3 mb-6">
                <div 
                  v-for="(stat, sIdx) in currentOfficer.stats" 
                  :key="sIdx"
                  class="p-3.5 rounded-xl bg-slate-50 dark:bg-neutral-900/60 border border-slate-200/70 dark:border-neutral-800/80 flex flex-col"
                >
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  <span 
                    ...
                    :key="tIdx"
                    class="text-xs font-semibold px-3 py-1 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-300 border border-blue-200/80 dark:border-blue-500/20"
                  >
                    {{ tag }}
                  </span>
                </div>
